...
def Inversa_matriz():
    ventana.destroy()
    subprocess.run(['python', 'PROYECTO_FINAL/Algebra/Inversa.py'])

def Multi_matrices():
    ventana.destroy()
    subprocess.run(['python', 'PROYECTO_FINAL/Algebra/Resolucion_Ecuaciones.py'])

def Ecua_lineales():
    ventana.destroy()
    subprocess.run(['python', 'PROYECTO_FINAL/Algebra/ecuaciones_lineales.py'])
...
ventana.title("PROYECTO ALGEBRA LINEAL")
ventana.state('zoomed')
...
titulo = tk.Label(
    ventana,
    text="RESOLUCIÓN DE ECUACIONES Y MATRICES",
    font=titulo_font,
    bg="#ffffff",
    fg="#000000"
)
...
# --- Botones principales ---
btn1 = crear_boton(ventana, "Inversa de una matriz", Inversa_matriz)
btn1.grid(row=2, column=0, columnspan=5, pady=15)

btn2 = crear_boton(ventana, "Multiplicación de matrices", Multi_matrices)
btn2.grid(row=3, column=0, columnspan=5, pady=15)

btn3 = crear_boton(ventana, "Ecuaciones lineales", Ecua_lineales)
